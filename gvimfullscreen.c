...
LONG _declspec(dllexport) ToggleFullScreen()
{
	...
	if (hTop)
	{
		...
		if ( GetWindowLong(hTop, GWL_STYLE) & WS_CAPTION )
		{
			...
			g_dy = mi.rcMonitor.bottom; // + 25 if you want complete full screen experience.
                                  // but my computer has an artifact on the lower left corner so I don't want to see it
...
BOOL CALLBACK EnumChildProc(HWND hwnd, LPARAM lParam)
{
	...
	if ( strcmp(lpszClassName, "VimTextArea") == 0 )
	{
		...
		SetWindowPos(hwnd, HWND_TOP, 0, 38, g_dx, g_dy, SWP_SHOWWINDOW);
